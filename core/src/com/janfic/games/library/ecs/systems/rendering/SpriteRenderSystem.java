...
public class SpriteRenderSystem extends SortedIteratingSystem {
    ...
    protected void processEntity(Entity entity, float deltaTime) {
        ...
        if(stackComponent != null) {
            ...
            stack.setStackRotation(10 * deltaTime + stack.getRotation());
            for (int i = 0; i < stack.getSprites().length; i++) {
                Sprite sprite = stackComponent.spriteStack.getSprites()[i];
                batch.draw(sprite, s.x - sprite.getOriginX(), s.y - sprite.getOriginY() + (i * stack.getLayerWidth()) , sprite.getOriginX(), sprite.getOriginY(), sprite.getWidth(), sprite.getHeight(), 1f, 1f,  stack.getRotation() + sprite.getRotation());
            }
        }
        if(shaderComponent != null) {
